...
string  type::prettyPrint ( const string& decl, const string& name )
{
    ...
    {
        string isconst;
        if ( _subtype->constness()) isconst="const ";

        if ( _subtype->code() == Array || _subtype->isFct() )
        {
            return isconst + _subtype->prettyPrint ( "(*" + decl + name + ")" ,"" ) ;
        }
        else
        {
            return isconst + _subtype->prettyPrint ( "*" + name  + decl ,"" ) ;
        }
    }
    else if ( _code == Array )
    {
        ostringstream s ;
        string isconst;
        if ( constness() ) isconst="const ";

        s << name << "[" << arraySize() << "]" ;
        return  isconst +  _subtype->prettyPrint ( decl + s.str(),"" ) ;
    }
    else if ( isFct() )
    {
        string tmp = name + "(" ;
